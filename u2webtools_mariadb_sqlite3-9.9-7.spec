...
Name: u2webtools_mariadb_sqlite3
Provides: u2webtools
Version: SETVERSION
Release: SETRELEASE
License: GPL
...
Conflicts: u2webtools_nomysql, u2webtools_mysql, u2webtools_mariadb
#DEB#Section: httpd

%description
Unix2Web-Http/Https-Server
unix2webd with MariaDB extension and SQlite3 extension, httpget, code_http
...
%build
#MAKEDEFINE#MYSQL="_mariadb" SQLITE3="_sqlite3"
if [[ -d SRC ]]; then
  if cd SRC; then
    for i in *; do
      (cd $i && MYSQL="_mariadb" SQLITE3="_sqlite3" make)
    done
  fi
fi
...
if [[ -d SRC ]]; then
  if cd SRC; then
    for i in *; do
      (cd $i && MYSQL="_mariadb" SQLITE3="_sqlite3" PREFIX=${RPM_BUILD_ROOT:-/dev/shm/u2w_makerpm}/usr make install)
    done
  fi
fi
